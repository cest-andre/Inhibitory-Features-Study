...
##
    ...
    # ax.scatter(np.arange(len(all_neg_percs)), np.array(all_neg_percs))
    # ax.set_xlabel("Neuron")
    # ax.set_ylabel("%")
    # ax.set_title(f"All Neg Channel Inputs over Total Neg Inputs\n{args.network} Layer {args.layer}")

    # plt.savefig(f'/home/andrelongon/Documents/inhibition_code/plots/perc_all_neg_channels/protos/{args.network}_{args.layer}.png', dpi=128)
    # plt.close()

    # f_stat, p_val = f_oneway(np.array(all_mean_most_weights), np.array(all_mean_least_weights))
    # print(f"{args.network} {args.layer} ANOVA results: F Score={f_stat}, p={p_val}")

    # ax.scatter(np.arange(len(all_mean_most_weights)), np.array(all_mean_most_weights), color='g', label="Most Corr")
    # ax.scatter(np.arange(len(all_mean_least_weights)), np.array(all_mean_least_weights), color='r', label="Least Corr")
    # ax.legend(loc="upper right")
    # ax.set_xlabel("Neuron")
    # ax.set_ylabel("Channel Weight (summed)")
    # ax.set_title(f"Weights of Top Corr Unit of Top Positive Weight\n{args.network} Layer {args.layer}")

    # plt.savefig(f'/home/andrelongon/Documents/inhibition_code/plots/corr_weights/pos/top1/{args.network}_{args.layer}_1pos_1sim.png', dpi=128)
    # plt.close()

    # ax.scatter(np.arange(len(all_pos_corrs)), np.array(all_pos_corrs), color='g', label="Top Pos")
    # ax.scatter(np.arange(len(all_neg_corrs)), np.array(all_neg_corrs), color='r', label="Top Neg")
    # ax.legend(loc="upper right")
    # ax.set_xlabel("Neuron")
    # ax.set_ylabel("Pearson r")
    # ax.set_title(f"Top Pos and Neg Curve Corr with Target\n{args.network} Layer {args.layer}")

    # plt.savefig(f'/home/andrelongon/Documents/inhibition_code/plots/top_pos_neg_target_corrs/no_clip_no_norm/{args.network}_{args.layer}.png', dpi=128)
    # plt.close()
    ...
    #     min_max_d, rand_d = max_input_proto_compare(extractor, topdir, botdir, protodir, antiprotodir, poslucentdir, neglucentdir,
    #         args.input_layer_name, args.weight_name, i, args.network, metric, compare_layer="layer2.1.BatchNorm2dbn1")

    #     if min_max_d is not None:
    #         all_min_max_d.append(min_max_d)
    #         all_rand_d.append(rand_d)

    # input_proto_sim_plot(all_min_max_d, all_rand_d, args.layer)


    # all_same_d = []
    # all_rand_d = []
    # for i in range(256):
    #     same_d, rand_d = proto_antiproto_compare(metric, args.network, args.layer, i)
    #     all_same_d.append(same_d)
    #     all_rand_d.append(rand_d)

    # proto_anti_sim_plot(all_same_d, all_rand_d, args.layer)
    ...
    # savedir = "/home/andrelongon/Documents/inhibition_code/max_stim_inputs/intact_v_ablate/alexnet_trained"
    ...
    # # _, _, acts, _, _, _ =\
    # #     validate_tuning_curve_thingsvision(extractor, args.input_layer_name, subset=0.1, sort_acts=False)
    # # acts = torch.tensor(np.array(acts))#[:, :, 2:5, 2:5]
    # # acts = torch.clamp(acts, min=0, max=None)

    # #   Calculate middle index and offset based on shape of preflattened weights and acts.
    # intact_acts = torch.tensor(get_max_stim_acts(extractor, proto_imgs, args.input_layer_name))
    # ablated_acts = torch.tensor(get_max_stim_acts(extractor, abl_proto_imgs, args.input_layer_name))
    ...
    # plt.savefig(f"/home/andre/histograms/input_histos/{args.type}_{args.input_layer_name}.png")
